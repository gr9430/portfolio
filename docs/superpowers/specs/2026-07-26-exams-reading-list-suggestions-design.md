...
2. Spot-check a handful of flagged "reconsider" books by hand. The script's degree will almost always be *higher* than what the network graph page visibly shows for the same book, for two independent reasons that both apply even to an ordinary (non-anthology) book: (a) the page excludes any citation key promoted to a "recommended" hub node, or already linked to another real book via `citationKey`, from book-to-book edges entirely (`phd/exams/index.html`, `establishedKeys()`/the `established.has(key) || citationKeyToBookId[key]` guard) — the script's raw degree applies neither exclusion; (b) the page additionally filters displayed edges by `sharedCount >= minSharedThreshold` (default 2). Do not expect a match; expect script-degree ≥ page-displayed-connections, and treat a large gap as expected, not a bug. For an anthology parent (e.g. `debates-23`), there's a third, additional reason the numbers diverge — the page currently shows the parent node with zero edges of its own (its `citations` array is empty by convention) while the script folds all its chapters' citations up to it — spot-check those instead by manually summing the distinct external books each chapter connects to.
